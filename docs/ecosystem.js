...
export class EcosystemBinding{
 ...
 async probeFormula({interactive=false}={}){const base=localStorage.getItem("LEEWAY_FORMULA_BASE_URL")||"http://127.0.0.1:4001",path=this.state.formulaConsumer?.endpoints?.health?.path||"/runtime/formula/v1/health";try{let permission="UNKNOWN";if(location.protocol==="https:"&&/^http:\/\/(127\.0\.0\.1|localhost)(:\d+)?$/i.test(base)&&navigator.permissions?.query){try{permission=(await navigator.permissions.query({name:"loopback-network"})).state}catch{permission="UNSUPPORTED"}}this.evidence("FORMULA_LOCAL_ACCESS",{base,permission,interactive});const c=new AbortController();setTimeout(()=>c.abort(),interactive?8000:1500);const request=new Request(base+path,{method:"GET",mode:"cors",cache:"no-store",signal:c.signal,targetAddressSpace:"loopback"});const r=await fetch(request);if(!r.ok)throw new Error("HTTP "+r.status);const h=await r.json(),pin=this.state.formulaAuthority?.pinnedSha256?.engine;this.state.formulaHealth=h;this.state.formulaServiceIdentity=(pin&&h.implementationSha256===pin&&h.loaded===true&&h.goldenVectorPass===true)?"VERIFIED":"MISMATCH";this.evidence("FORMULA_HEALTH",{identity:this.state.formulaServiceIdentity,implementationSha256:h.implementationSha256,goldenVectorPass:h.goldenVectorPass,execution:"NOT_EXECUTED"})}catch(e){this.state.formulaServiceIdentity="UNEXPOSED";this.evidence("FORMULA_HEALTH",{identity:"UNEXPOSED",message:e.message,execution:"NOT_EXECUTED",interactive})}return this.state.formulaServiceIdentity}
